...
func delDigits(_ string: String) -> String {
    string.filter( { !$0.isDigit() })
}

print(delDigits("Ala3s, poo4r Yoric888k! I kn3ew h41im, H0or4at1i0o."))

func delImbeddedDigits(_ string: String) -> String {
    let wordArray = string.split(separator: " ")
    var cleanWordArray: [String] = []
    
    for word in wordArray {
        if word.first!.isLetter {
            cleanWordArray.append(delDigits(String(word)))
        } else {
            cleanWordArray.append(String(word))
        }
    }
    return cleanWordArray.joined(separator: " ")
}

print(delImbeddedDigits("Ple3ase turn to page 71."))
