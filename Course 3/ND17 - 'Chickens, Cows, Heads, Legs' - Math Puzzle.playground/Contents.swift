...
func headsAndLegs(heads: Int, legs: Int) -> [Int]? {
    
    let maxLegs = heads * 4
    let numberOfChickens = (maxLegs - legs) / 2
    let numberOfCows = heads - numberOfChickens
    
    if numberOfChickens * 2 + numberOfCows * 4 == legs && numberOfChickens >= 0 && numberOfCows >= 0 {
        return [numberOfChickens, numberOfCows]
    }
    return nil
}

print(headsAndLegs(heads: 72, legs: 200) ?? "no solution")
print(headsAndLegs(heads: 72, legs: 201) ?? "no solution")
print(headsAndLegs(heads: 73, legs: 200) ?? "no solution")

func headsAndLegsWithSheep(heads: Int, legs: Int, numberOfSheep: Int) -> [Int]? {
    let reducedHeads = heads - numberOfSheep
    let reducedLegs = legs - numberOfSheep * 4
    return headsAndLegs(heads: reducedHeads, legs: reducedLegs)
}
