...
extension Character {
    func sameCaseAs(_ otherChar: Character) -> Int {
        guard self.isLetter && otherChar.isLetter else { return -1 }
        let isUpper: Bool = self == Character(self.uppercased())
        
        if isUpper {
            if otherChar == Character(otherChar.uppercased()) {
                return 1
            } else {
                return 0
            }
        }
        if otherChar == Character(otherChar.lowercased()) {
            return 1
        } else {
            return 0
        }
