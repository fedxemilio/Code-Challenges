...
func herosFormula(sideA: Double, sideB: Double, sideC: Double) -> String {
    let s: Double = (sideA + sideB + sideC) / 2
    let area = sqrt(s * (s - sideA) * (s - sideB) * (s - sideC))
    return String(format: "%.2f", area)
}

print(herosFormula(sideA: 4, sideB: 13, sideC: 15))

func isValidTriangle(sideA: Double, sideB: Double, sideC: Double) -> Bool {
    return sideA + sideB > sideC && sideA + sideC > sideB && sideB + sideC > sideA
}

print(isValidTriangle(sideA: 4, sideB: 13, sideC: 15))
print(isValidTriangle(sideA: 2, sideB: 13, sideC: 15))
