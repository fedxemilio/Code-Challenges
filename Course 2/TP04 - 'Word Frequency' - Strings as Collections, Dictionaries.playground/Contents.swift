...
func wordCount(_ line: String) -> [String: Int] {
    var wordDict: [String: Int]  = [:]
    let words = line.split(separator: " ")
    for word in words {
        let count = words.count {$0.lowercased() == word.lowercased()}
        wordDict[String(word.lowercased())] = count
    }
    return wordDict
}

let line1 = "Beware the Jabberwock my son The jaws that bite The claws that catch"

print(wordCount(line1))

let alphabet = "abcdefghijklmnopqrstuvwxyz"

func wordCount2(_ line: String) -> [String: Int] {
    var wordDict: [String: Int]  = [:]
    let words = line.split(separator: " ")
    for word in words {
        let count = words.count {$0.lowercased() == word.lowercased()}
